setwd("C:/Users/Vojtìch Pulec/OneDrive/Dokumenty/HU/Thesis/master-thesis_Pulec") # thats for me to get to proper level
wd <- getwd()
wd <- paste(wd, "/data/03_final_sample/Google/ethereum", sep = "")
setwd(wd)
rm(list=ls())
...
# *** task bar
pb <- tkProgressBar(title = "progress bar", min = 0, max = nfiles, width = 100)
...
average=mean((as.numeric(intersect[,2]))/(as.numeric(intersect[,3])))

#progress bar
setTkProgressBar(pb, i, label=paste( round(i/nrow(nfiles)*100 , 0),"% done"))
