...
wd <- paste(wd, "/Kraken/ETH", sep = "")
setwd(wd)
load(file = "XETHZEUR.RData")

# *** trasnforming into wide data
prices <- as.data.frame(vector(mode = "numeric",length= nrow(XETHZEUR)))
...
save(minute_data, file="ETH_1minute.RData")

require(ggplot2)

ggplot(minute_data, aes(minute_data$time)) +                                      # basic graphical object
  geom_line(aes(y=minute_data$SVI*3), colour="red") +             # first layer
