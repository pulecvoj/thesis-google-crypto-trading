...
fifteen_minute_data$crix <- 0

for (i in 1:nrow(fifteen_minute_data)) {
  fifteen_minute_data$crix[i] <- CRIX_15minute_data$Index[match(fifteen_minute_data$time[i], CRIX_15minute_data$timestamp)]
...
plot(fifteen_minute_data$crix)
sum(is.na(fifteen_minute_data$crix))/length(fifteen_minute_data$crix)