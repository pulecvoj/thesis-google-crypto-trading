...
nfiles <- length(as.vector(files))-2
...
# *** task bar
pb <- tkProgressBar(title = "progress bar", min = 0, max = nfiles, width = 300)
...
p1=merged[,-3] 
colnames(p1)=c("date",'rank') 
setTkProgressBar(pb, i, label=paste( round(i/nfiles*100 , 0),"% done"))
}
