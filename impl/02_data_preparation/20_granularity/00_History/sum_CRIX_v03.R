...
for( i in 2:nrow(CRIX)){
CRIX$diff_round[i] <- CRIX$round_unix[i] - CRIX$round_unix[i-1]
paste(i)
}
...
prev <- CRIX$round_unix
CRIX_30minute_data$value <- aggregate(CRIX$price, list(prev), sum, na.rm = TRUE)[,2]/6 # mean of CRIX
...
prev <- CRIX$round_unix
CRIX_60minute_data$value <- aggregate(CRIX$price, list(prev), sum, na.rm = TRUE)[,2]/12 # mean of CRIX
...
prev <- CRIX$round_unix
CRIX_day_data$value <- aggregate(CRIX$price, list(prev), sum, na.rm = TRUE)[,2]/288 # mean of CRIX
