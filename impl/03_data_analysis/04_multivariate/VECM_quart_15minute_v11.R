...
for(w in clist){
  ...
  for(q in c(48,96,288,672)){
    ...
    for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
      sbst <- as.matrix(fifteen_minute_data[c(i:(i+shift)), c(19,21,27,26)])
      sbst_dum <- as.matrix(fifteen_minute_data[c(i:(i+shift)), c(22,23)])
      ...
      # buy / sell decision
      if(is.na(fifteen_minute_data$A_m1[i + shift])) {
      ...
      #constant
      fifteen_minute_data$alpha[i + shift] <- fit$deterministic[1,1]
      fifteen_minute_data$zeta[i + shift] <- fit$deterministic[1,2]
      fifteen_minute_data$eta[i + shift] <- fit$deterministic[1,3]
      ...
      fifteen_minute_data$fi[i + shift] <- fit$A$A1[1,3]
      
      #SVI positive second lag
      fifteen_minute_data$fi_l2[i + shift] <- fit$A$A2[1,3]
      
      #SVI positive third lag
      t <- try(fifteen_minute_data$fi_l3[i + shift] <- fit$A$A3[1,3])
      if(inherits(t, "try-error")){
        fifteen_minute_data$fi_l3[i + shift] <- NA
      } else {
        fifteen_minute_data$fi_l3[i + shift] <- t
      }
      
      #SVI negative first lag
      fifteen_minute_data$theta[i + shift] <- fit$A$A1[1,4]
      
      #SVI negative second lag
      fifteen_minute_data$theta_l2[i + shift] <- fit$A$A2[1,4]
      
      #SVI negative third lag
      t <- try(fifteen_minute_data$theta_l3[i + shift] <- fit$A$A3[1,4])
      if(inherits(t, "try-error")){
        fifteen_minute_data$theta_l3[i + shift] <- NA
      } else {
        fifteen_minute_data$theta_l3[i + shift] <- t
      }
    ...
    save(fifteen_minute_data,file=paste(w,"_15minut_VECM_quart_fit_",lookback ,".Rdata",sep = ""))
