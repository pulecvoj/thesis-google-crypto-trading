...
for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
  ...
  fifteen_minute_data$beta[i + shift] <- fit$A$A1[1,1] #autoregressive first lag
  fifteen_minute_data$beta_l2[i + shift] <- fit$A$A2[1,1] #autoregressive second lag
  fifteen_minute_data$beta_l3[i + shift] <- fit$A$A3[1,1] #autoregressive third lag
  
  fifteen_minute_data$gamma[i + shift] <- fit$A$A1[1,2] #SVI first lag
  fifteen_minute_data$gamma_l2[i + shift] <- fit$A$A2[1,2] #SVI second lag
  fifteen_minute_data$gamma_l3[i + shift] <- fit$A$A3[1,2] #SVI third lag
