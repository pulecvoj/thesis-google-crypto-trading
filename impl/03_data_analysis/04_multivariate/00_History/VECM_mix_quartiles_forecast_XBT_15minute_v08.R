...
# *** loading data ****************************
  load(file="XBT_15minute_returns_daily_mix_quartiles.Rdata")
  ...
  colnames(fifteen_minute_data)[61] <- c("Lag")
...
for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
  sbst <- as.matrix(fifteen_minute_data[c(i:(i+shift)), c(18,21,22,23)])
  ...
  #recording lags
  fifteen_minute_data[i+shift,61] <- lags
  
  #setTkProgressBar(pb, i, label=paste( round(i/(nrow(fifteen_minute_data)-timeframe)*1000, 0),"‰ done"))
  
  print(paste("XBT","_", "daily_mix_quartiles","_",q, "_",i))
  
  }

save(fifteen_minute_data,file=paste(window_start,"_", window_end,"_", lookback,"_15minut_VAR_daily_mix_quartiles_fit_XBT.Rdata", sep = ""))
}

#********************************************** end of fitting ***************************************************************

forecasts <- fifteen_minute_data[which(fifteen_minute_data$A_m1 !=1), ]
forecasts <- forecasts[which(!is.na(forecasts$A_m1)), ]
time_vector <- forecasts$time
save(time_vector,file=paste(window_start,"_", window_end,"_", lookback,"_time_vector_15minut_VAR_daily_mix_quartiles_fit_XBT.RData", sep = ""))
