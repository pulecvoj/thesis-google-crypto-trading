...
# *** loading data ****************************
  load(file="XBT_15minute_returns_daily_diff_binary.Rdata")
...
for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
  sbst <- as.matrix(fifteen_minute_data[c(i:(i+shift)), c(18,21,22)])
  ...
  print(paste("XBT","_", "daily_diff_binary","_",q, "_",i))
  
  }

save(fifteen_minute_data,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_daily_diff_binary_fit_XBT.Rdata", sep = ""))
}
