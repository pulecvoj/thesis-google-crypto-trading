...
# *** loading data ****************************
  load(file="ETH_1minute_returns_hourly_diff_mix_quartiles.Rdata")
  ...
  minute_data[c(is.infinite(minute_data[,21])),21] <- 0 
  minute_data[c(is.na(minute_data[,21])),21] <- 0 
  minute_data[c(is.nan(minute_data[,21])),21] <- 0 
  
  minute_data[c(is.infinite(minute_data[,22])),22] <- 0 
  minute_data[c(is.na(minute_data[,22])),22] <- 0 
  minute_data[c(is.nan(minute_data[,22])),22] <- 0 
  
  minute_data[c(is.infinite(minute_data[,23])),23] <- 0 
  minute_data[c(is.na(minute_data[,23])),23] <- 0 
  minute_data[c(is.nan(minute_data[,23])),23] <- 0 
...
for(i in 1:(nrow(minute_data)-timeframe)){
  sbst <- as.matrix(minute_data[c(i:(i+shift)), c(18,21,22,23)])
  ...
  print(paste("ETH","_", "hourly_Diff_mix_quartiles","_",q, "_",i))
  
  }

save(minute_data,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_hourly_diff_mix_quartiles_fit_ETH.Rdata", sep = ""))
...
# 
# load(file=paste(window_start,"_", window_end,"_", lookback,"_time_vector_one_minut_ARIMA_fit_ETH.Rdata", sep = ""))
# 
...
# 
# 
# save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_no_diff_benchmark_ETH.Rdata", sep = ""))
...
save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_no_diff_solely_benchmark_ETH.Rdata", sep = ""))
