...
for (i in 3:nrow(five_minute_data)) {
...
# **** fitting var ******************************
sbst <- as.matrix(five_minute_data[c(window_start:window_end), c(18,21)])

fit <- VAR(sbst, p = 1, type = "none")
irf.var <- irf(fit, impulse = "search_negative_hourly", response = "logreturn_price_all",  n.ahead = 15, ci = 0.95)
plot(irf.var)
...
pdf("plot_neg_5mins_XBT_hourly_tred.pdf")
...
sbst <- as.matrix(five_minute_data[c(window_start:window_end), c(18,22)])

fit <- VAR(sbst, p = 1, type = "none")
irf.var <- irf(fit, impulse = "search_positive_hourly", response = "logreturn_price_all",  n.ahead = 15, ci = 0.95)
plot(irf.var)
