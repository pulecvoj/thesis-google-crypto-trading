...
#splitting search into positive and negative
  ...
  minute_data <- minute_data[c(window_start:window_end),]
  ...
  #preparing struncture - 25
  minute_data[,25:56] <- 1
  colnames(minute_data)[c(25,28,31,34,37)] <- c("A_m1","A_m3","A_m5","A_m10", "A_m15")                             #mean for x minutes forward estimate
  colnames(minute_data)[c(26,29,32,35,38)] <- c("A_u1(95%)", "A_u3(95%)", "A_u5(95%)", "A_u10(95%)","A_u15(95%)")  #upper bound for x minutes forward estimate
  colnames(minute_data)[c(27,30,33,36,39)] <- c("A_l1(95%)" ,"A_l3(95%)", "A_l5(95%)", "A_l10(95%)", "A_l15(95%)")  # lower bound for x minutes forward estimate
  colnames(minute_data)[c(40,41)] <- c("A_BIC", "A_AICC")                                                            # AIC a BIC criteria for choosing proper order
  colnames(minute_data)[c(42:45)] <- c("R_1","R_3","R_5","R_10", "R_15")                                          #real values
  colnames(minute_data)[c(46:51)] <- c("A_SQe_1","A_SQe_3","A_SQe_5","A_SQe_10", "A_SQe_15")                      #squared errors of estimate 
  colnames(minute_data)[c(52:56)] <- c("A_DA_1","A_DA_3","A_DA_5","A_DA_10", "A_DA_15")                           #directional accuracy
...
for(i in 1:(nrow(minute_data)-timeframe)){
  ...
  print(paste("XBT","_", "Hourly","_",q, "_",i))
...
save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_hourly_benchmark_XBT.Rdata", sep = ""))
...
save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_minut_VAR_hourly_solely_benchmark_XBT.Rdata", sep = ""))
