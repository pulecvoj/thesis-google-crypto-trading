...
# *** loading data ****************************
  load(file=paste(w,"_15minute_returns_bin.Rdata",sep = ""))
...
for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
  sbst <- as.matrix(fifteen_minute_data[c(i:(i+shift)), c(19,21,27)])
  sbst_dum <- as.matrix(fifteen_minute_data[c(i:(i+shift)), 22])
...
save(fifteen_minute_data,file=paste(w,"_15minut_VECM_bin_fit_",lookback ,".Rdata",sep = ""))
