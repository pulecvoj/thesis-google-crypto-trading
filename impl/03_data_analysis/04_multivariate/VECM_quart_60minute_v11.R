...
for(w in clist){
  ...
  for(q in c(48,96,168,336)){
    ...
    for(i in 1:(nrow(hour_data)-timeframe)){
      sbst <- as.matrix(hour_data[c(i:(i+shift)), c(19,21,27,26)])
      sbst_dum <- as.matrix(hour_data[c(i:(i+shift)), c(22,23)])
      ...
      hour_data$eta[i + shift] <- fit$deterministic[1,3]
      ...
      hour_data$theta[i + shift] <- fit$A$A1[1,4]
      
      #SVI negative second lag
      hour_data$theta_l2[i + shift] <- fit$A$A2[1,4]
      
      #SVI negative third lag
      t <- try(hour_data$theta_l3[i + shift] <- fit$A$A3[1,4])
      if(inherits(t, "try-error")){
        hour_data$theta_l3[i + shift] <- NA
      } else {
        hour_data$theta_l3[i + shift] <- t
      }
    ...
    save(hour_data,file=paste(w,"_60minut_VECM_quart_fit_",lookback ,".Rdata",sep = ""))
