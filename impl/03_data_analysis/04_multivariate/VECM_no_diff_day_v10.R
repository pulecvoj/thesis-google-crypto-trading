...
for(i in 1:(nrow(day_data)-timeframe)){
  sbst <- as.matrix(day_data[c(i:(i+shift)), c(19,21)])
  ...
  day_data$alpha[i + shift] <- fit$deterministic[1] #constant
...
save(day_data,file=paste(w,"_day_VECM_no_diff_fit_",lookback ,".Rdata",sep = ""))
