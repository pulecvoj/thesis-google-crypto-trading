...
for(w in clist){
  ...
  for(q in c(14,21,28,42)){
    ...
    for(i in 1:(nrow(day_data)-timeframe)){
      sbst <- as.matrix(day_data[c(i:(i+shift)), c(19,21,27,26)])
      sbst_dum <- as.matrix(day_data[c(i:(i+shift)), c(22,23)])
      ...
      day_data$eta[i + shift] <- fit$deterministic[1,3]
      ...
      day_data$theta[i + shift] <- fit$A$A1[1,4]
      
      #SVI negative second lag
      day_data$theta_l2[i + shift] <- fit$A$A2[1,4]
      
      #SVI negative third lag
      t <- try(day_data$theta_l3[i + shift] <- fit$A$A3[1,4])
      if(inherits(t, "try-error")){
        day_data$theta_l3[i + shift] <- NA
      } else {
        day_data$theta_l3[i + shift] <- t
      }
    ...
    save(day_data,file=paste(w,"_day_VECM_quart_fit_",lookback ,".Rdata",sep = ""))
