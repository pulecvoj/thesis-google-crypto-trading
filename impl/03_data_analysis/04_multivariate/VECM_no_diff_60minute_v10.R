...
for(i in 1:(nrow(hour_data)-timeframe)){
  sbst <- as.matrix(hour_data[c(i:(i+shift)), c(19,21)])
  ...
  t <- try(predict(fit, n.ahead = 15, ic = "AIC", ci = 0.95))
  if(inherits(t, "try-error")){
    next()
  } else {
    forecast60 <-  t
  ...
  hour_data$alpha[i + shift] <- fit$deterministic[1] #constant
...
save(hour_data,file=paste(w,"_60minut_VECM_no_diff_fit_",lookback ,".Rdata",sep = ""))
