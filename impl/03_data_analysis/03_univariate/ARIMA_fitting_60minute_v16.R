...
#setwd("C:/Users/Vojtìch Pulec/OneDrive/Dokumenty/HU/Thesis/master-thesis_Pulec") # thats for me to get to proprer level
  setwd("C:/Users/vojta/OneDrive/Dokumenty/HU/Thesis/master-thesis_Pulec") # thats for me to get to proprer level
wd <- getwd()
...
for(q in c(48,96,168,336)){
  ...
  # 1) *********** data prep ******************************************
  load(file=paste(w,"_60minute_returns_quart.Rdata",sep = ""))
  ...
  colnames(hour_data)[c(61:63)] <- c("p","d","q")                                                                  #orders 
  ...
  colnames(hour_data)[69] <- "WN_RW_indication"
  ...
  #pb <- tkProgressBar(title = "progress bar", min = 0, max = (nrow(hour_data)- timeframe), width = 1000)
  ...
  for(i in 1:(nrow(hour_data)-timeframe)){
    ...
    fit <- auto.arima(hour_data[(i:(i+shift)),19])
    forecast60 <- forecast(fit, h=15)
    # fitting 1 step ahead
    hour_data$A_m1[i+shift] <- forecast60$mean[1] + 1
    hour_data$A_u1[i+shift] <- forecast60$upper[1,2] + 1
    hour_data$A_l1[i+shift] <- forecast60$lower[1,2] + 1
    
    
    # fitting 3 steps ahead
    for(z in 1:3){
      hour_data$A_m3[i+shift] <- (forecast60$mean[z] + 1) *  hour_data$A_m3[i+shift] 
      hour_data$A_u3[i+shift] <- (forecast60$upper[z,2] + 1) * hour_data$A_u3[i+shift] 
      hour_data$A_l3[i+shift] <- (forecast60$lower[z,2] + 1) * hour_data$A_l3[i+shift]
    }
    
    # fitting 5 steps ahead
    for(z in 1:5){
      hour_data$A_m5[i+shift] <- (forecast60$mean[z] + 1) *  hour_data$A_m5[i+shift]
      hour_data$A_u5[i+shift] <- (forecast60$upper[z,2] + 1) * hour_data$A_u5[i+shift]
      hour_data$A_l5[i+shift] <- (forecast60$lower[z,2] + 1) * hour_data$A_l5[i+shift]
    }
    
    # fitting 10 steps ahead
    for(z in 1:10){
      hour_data$A_m10[i+shift] <- (forecast60$mean[z] + 1) *  hour_data$A_m10[i+shift]
      hour_data$A_u10[i+shift] <- (forecast60$upper[z,2] + 1) * hour_data$A_u10[i+shift]
      hour_data$A_l10[i+shift] <- (forecast60$lower[z,2] + 1) * hour_data$A_l10[i+shift]
    }
    
    # fitting 15 steps ahead
    for(z in 1:15){
      hour_data$A_m15[i+shift] <- (forecast60$mean[z] + 1) *  hour_data$A_m15[i+shift]
      hour_data$A_u15[i+shift] <- (forecast60$upper[z,2] + 1) *  hour_data$A_u15[i+shift]
      hour_data$A_l15[i+shift] <- (forecast60$lower[z,2] + 1) * hour_data$A_l15[i+shift]
    }
    
    hour_data$A_BIC[i+shift] <- forecast60$model$bic
    hour_data$A_AICC[i+shift] <- forecast60$model$aicc
    ...
    #extracting lags
    hour_data$p[i+shift] <- arimaorder(fit)[1]
    hour_data$d[i+shift] <- arimaorder(fit)[2]
    hour_data$q[i+shift] <- arimaorder(fit)[3]
    ...
    hour_data$WN_RW_indication[i+shift] <- hour_data$p[i+shift] + hour_data$q[i+shift]
    
    #extracting model parameters
    hour_data$alpha[i+shift] <- fit$coef[2]
    hour_data$beta[i+shift] <- fit$coef[1] * (hour_data$p[i+shift] > 0)
    
    # buy / sell decision
  if(hour_data$A_m1[i + shift] < 1)
  ...
  save(hour_data,file=paste(w,"_60minut_ARIMA_fit_",lookback ,".Rdata",sep = ""))

}
}
    
  forecasts <- hour_data[which(hour_data$WN_RW_indication !=0), ]
  time_vector <- forecasts$time
  save(time_vector,file=paste(window_start,"_", window_end,"_", lookback,"_time_vector_60minut_ARIMA_fit_XBT.Rdata", sep = ""))
  
  
  
  #actual returns share
  returns_benchmark <- hour_data[which(hour_data$WN_RW_indication !=0), ]
  returns_benchmark$one  <- nrow(returns_benchmark[which(returns_benchmark$R_1 > 1),])/(nrow(returns_benchmark)-sum(is.na(hour_data$A_m1)))*100
  returns_benchmark$three <- nrow(returns_benchmark[which(returns_benchmark$R_3 > 1),])/(nrow(returns_benchmark)-sum(is.na(hour_data$A_m3)))*100
  returns_benchmark$five <- nrow(returns_benchmark[which(returns_benchmark$R_5 > 1),])/(nrow(returns_benchmark)-sum(is.na(hour_data$A_m5)))*100
  returns_benchmark$ten <- nrow(returns_benchmark[which(returns_benchmark$R_10 > 1),])/(nrow(returns_benchmark)-sum(is.na(hour_data$A_m10)))*100
  returns_benchmark$fifteen  <- nrow(returns_benchmark[which(returns_benchmark$R_15 > 1),])/(nrow(returns_benchmark)-sum(is.na(hour_data$A_m15)))*100
  ...
  save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_60minut_ARIMA_benchmark_XBT.Rdata", sep = ""))
  ...
  diagnostics$WN_share <- 1 - (nrow(forecasts)/nrow(hour_data))
  
  save(diagnostics,file=paste(window_start,"_", window_end,"_", lookback,"_60minut_ARIMA_diagnostics_XBT.Rdata", sep = ""))
