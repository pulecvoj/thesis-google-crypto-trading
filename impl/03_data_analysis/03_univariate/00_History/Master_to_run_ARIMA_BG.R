...
for(q in c(60,120,240,360)){
  ...
  for(i in 1:(nrow(minute_data)-timeframe)){
    ...
    #testing
    minute_data[i+shift,54] <- Box.test (fit$residuals, lag = 10)[3]
    minute_data[i+shift,55] <- shapiro.test(fit$residuals)[2]
    minute_data[i+shift,56] <- lillie.test(fit$residuals)[2]
    minute_data[i+shift,57] <- t.test(fit$residuals)[3]
    minute_data[i+shift,58] <- Box.test (fit$residuals^2, lag = 10)[3]
