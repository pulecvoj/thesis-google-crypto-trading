...
# Progress bar
 ...
 save(minute_data,file=paste(window_start,"_", window_end,"_", lookback,"_one_minut_ARIMA_fit_XBT.Rdata", sep = ""))
 
 forecasts <- minute_data[which(minute_data$A_m1 !=1),]
 time_vector <- forecasts$time
 save(time_vector,file=paste(window_start,"_", window_end,"_", lookback,"_time_vector_one_minut_ARIMA_fit_XBT.Rdata", sep = ""))
 ...
 save(returns_benchmark,file=paste(window_start,"_", window_end,"_", lookback,"_minut_ARIMA_benchmark_XBT.Rdata", sep = ""))
 ...
 # **********  5 minute*******************************************
 ggplot(minute_data, aes(minute_data$time)) +                                    
   geom_line(aes(y=minute_data$A_m1), colour="red") +               #estimates         
   geom_line(aes(y=minute_data$R_1), colour="green")+               #real
   geom_line(aes(y=minute_data$`A_u1(95%)`), colour="black")+       #upper estimated bound
   geom_line(aes(y=minute_data$`A_l1(95%)`), colour="black")        #lower estimated bound
 
 # **********  10 minute*******************************************
 ggplot(minute_data, aes(minute_data$time)) +                                    
   geom_line(aes(y=minute_data$A_m3), colour="red") +               #estimates         
   geom_line(aes(y=minute_data$R_3), colour="green")+               #real
   geom_line(aes(y=minute_data$`A_u3(95%)`), colour="black")+       #upper estimated bound
   geom_line(aes(y=minute_data$`A_l3(95%)`), colour="black")        #lower estimated bound
 
 
 
 # **********  15 minute*******************************************
 ggplot(minute_data, aes(minute_data$time)) +                                    
   geom_line(aes(y=minute_data$A_m5), colour="red") +               #estimates         
   geom_line(aes(y=minute_data$R_5), colour="green")+               #real
   geom_line(aes(y=minute_data$`A_u5(95%)`), colour="black")+       #upper estimated bound
   geom_line(aes(y=minute_data$`A_l5(95%)`), colour="black")        #lower estimated bound
 
 # **********  30 minute*******************************************
 ggplot(minute_data, aes(minute_data$time)) +                                    
   geom_line(aes(y=minute_data$A_m10), colour="red") +               #estimates         
   geom_line(aes(y=minute_data$R_10), colour="green")+               #real
   geom_line(aes(y=minute_data$`A_u10(95%)`), colour="black")+       #upper estimated bound
   geom_line(aes(y=minute_data$`A_l10(95%)`), colour="black")        #lower estimated bound
 
 # **********  60 minute*******************************************
 ggplot(minute_data, aes(minute_data$time)) +                                    
   geom_line(aes(y=minute_data$A_m15), colour="red") +               #estimates         
   geom_line(aes(y=minute_data$R_15), colour="green")+               #real
   geom_line(aes(y=minute_data$`A_u15(95%)`), colour="black")+       #upper estimated bound
   geom_line(aes(y=minute_data$`A_l15(95%)`), colour="black")        #lower estimated bound