...
for(q in c(48,96,288,672)){
  ...
  colnames(fifteen_minute_data)[c(61:63)] <- c("p","d","q")                                                                  #orders 
  ...
  colnames(fifteen_minute_data)[69] <- "WN_RW_indication"
  ...
  fifteen_minute_data$eur[shift] <- 1000 #initial 1000 eur to trade with
  fifteen_minute_data$taker26_eur[shift] <- 1000 #initial 1000 eur to trade with
  fifteen_minute_data$taker10_eur[shift] <- 1000 #initial 1000 eur to trade with  
  ...
  for(i in 1:(nrow(fifteen_minute_data)-timeframe)){
    ...
    fit <- auto.arima(fifteen_minute_data[(i:(i+shift)),18])
    forecast60 <- forecast(fit, h=15)
    # fitting 1 step ahead
    fifteen_minute_data$A_m1[i+shift] <- forecast60$mean[1] + 1
    fifteen_minute_data$A_u1[i+shift] <- forecast60$upper[1,2] + 1
    fifteen_minute_data$A_l1[i+shift] <- forecast60$lower[1,2] + 1
    
    
    # fitting 3 steps ahead
    for(z in 1:3){
      fifteen_minute_data$A_m3[i+shift] <- (forecast60$mean[z] + 1) *  fifteen_minute_data$A_m3[i+shift] 
      fifteen_minute_data$A_u3[i+shift] <- (forecast60$upper[z,2] + 1) * fifteen_minute_data$A_u3[i+shift] 
      fifteen_minute_data$A_l3[i+shift] <- (forecast60$lower[z,2] + 1) * fifteen_minute_data$A_l3[i+shift]
    }
    
    # fitting 5 steps ahead
    for(z in 1:5){
      fifteen_minute_data$A_m5[i+shift] <- (forecast60$mean[z] + 1) *  fifteen_minute_data$A_m5[i+shift]
      fifteen_minute_data$A_u5[i+shift] <- (forecast60$upper[z,2] + 1) * fifteen_minute_data$A_u5[i+shift]
      fifteen_minute_data$A_l5[i+shift] <- (forecast60$lower[z,2] + 1) * fifteen_minute_data$A_l5[i+shift]
    }
    
    # fitting 10 steps ahead
    for(z in 1:10){
      fifteen_minute_data$A_m10[i+shift] <- (forecast60$mean[z] + 1) *  fifteen_minute_data$A_m10[i+shift]
      fifteen_minute_data$A_u10[i+shift] <- (forecast60$upper[z,2] + 1) * fifteen_minute_data$A_u10[i+shift]
      fifteen_minute_data$A_l10[i+shift] <- (forecast60$lower[z,2] + 1) * fifteen_minute_data$A_l10[i+shift]
    }
    
    # fitting 15 steps ahead
    for(z in 1:15){
      fifteen_minute_data$A_m15[i+shift] <- (forecast60$mean[z] + 1) *  fifteen_minute_data$A_m15[i+shift]
      fifteen_minute_data$A_u15[i+shift] <- (forecast60$upper[z,2] + 1) *  fifteen_minute_data$A_u15[i+shift]
      fifteen_minute_data$A_l15[i+shift] <- (forecast60$lower[z,2] + 1) * fifteen_minute_data$A_l15[i+shift]
    }
    
    fifteen_minute_data$A_BIC[i+shift] <- forecast60$model$bic
    fifteen_minute_data$A_AICC[i+shift] <- forecast60$model$aicc
    ...
    #extracting lags
    fifteen_minute_data$p[i+shift] <- arimaorder(fit)[1]
    fifteen_minute_data$d[i+shift] <- arimaorder(fit)[2]
    fifteen_minute_data$q[i+shift] <- arimaorder(fit)[3]
    ...
    fifteen_minute_data$WN_RW_indication[i+shift] <- fifteen_minute_data$p[i+shift] + fifteen_minute_data$q[i+shift]
    
    #extracting model parameters
    fifteen_minute_data$alpha[i+shift] <- fit$coef[2]
    fifteen_minute_data$beta[i+shift] <- fit$coef[1] * (fifteen_minute_data$p[i+shift] > 0)
  ...
  par(mfrow=c(2,2))
  plot(fifteen_minute_data$price_all,pch=16,cex = 0.3)
  plot(fifteen_minute_data$eur,pch=16,cex = 0.3)
  plot(fifteen_minute_data$taker26_eur,pch=16,cex = 0.3)
  plot(fifteen_minute_data$taker10_eur,pch=16,cex = 0.3)
  
  save(fifteen_minute_data,file=paste(w,"_15minut_ARIMA_fit_",lookback ,".Rdata",sep = ""))
