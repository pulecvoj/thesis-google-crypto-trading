...
# *** task bar
pb <- tkProgressBar(title = "progress bar", min = 0, max = nrow(five_minute_data), width = 100)

window_start <- 100 #any number from number of lags to nrow-1
window_end <- 500 #any number from start to nrow

for (i in window_start:window_end){
  acf <- acf(five_minute_data$logreturn_price_all[i:(i+288)], lag.max = 5)
  time_acf[i,1] <- five_minute_data$time[i+288]
  for(z in 2:6){
    time_acf[i,z] <- acf$acf[z-1]
  }
  setTkProgressBar(pb, i, label=paste( round(i/(window_length)*100 , 0),"% done"))
}
