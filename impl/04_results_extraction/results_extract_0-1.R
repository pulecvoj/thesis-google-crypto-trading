...
gran <- "15minut"        #("15minut","30minut","60minut", "day")
learn <-  48    #(48,96,168,336,144,672,14,21,28,42)
model <-  "ARIMA_fit"      #("ARIMA_fit", "VECM_no_diff", "VECM_bin_fit", "VECM_quart_fit")
...
results[6,2] <- round(mean(fit$A_SQe_1)*1000,3)
results[7,2] <- round(mean(fit$A_SQe_3, na.rm = TRUE)*1000,3)
results[8,2] <- round(mean(fit$A_SQe_5, na.rm = TRUE)*1000,3)
results[9,2] <- round(mean(fit$A_SQe_10, na.rm = TRUE)*1000,3)
results[10,2] <- round(mean(fit$A_SQe_15, na.rm = TRUE)*1000,3)
results[11,2] <- round(nrow(fit)/nrow(fit_compl),3)
results[12,2] <- round(nrow(fit)/nrow(fit_compl),3)
results[13,2] <- "Profit_0.26"
results[14,2] <- "Profit_0.10"
